...
const badgeColor: Record<
  OrderStatus,
  "amber" | "blue" | "orange" | "green" | "red" | "gray"
> = {
  Pending: "amber",
  Confirmed: "blue",
  Preparing: "orange",
...
export default function DashboardPage() {
  ...
  const {
    ...
    isError: dailySalesError,
  } = useQuery({
    queryKey: ["dailySales", today],
    queryFn: () => getDailySales(today),
  });
  ...
  const statusCounts = useMemo(() => {
    ...
    (Object.keys(statusColor) as OrderStatus[]).forEach((s) =>
      counts.set(s, 0),
    );
  ...
  const finance = useMemo(() => {
    const dineIn =
      orders
        ?.filter((o) => o.orderType?.toLowerCase().includes("dine"))
        .reduce((s, o) => s + Number(o.totalAmount || 0), 0) ?? 0;

    const takeaway =
      orders
        ?.filter((o) => o.orderType?.toLowerCase().includes("take"))
        .reduce((s, o) => s + Number(o.totalAmount || 0), 0) ?? 0;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card
        as="section"
        title="Today's Sales"
        subtitle="Sales performance for today"
        action={
      ...
      >
        {dailySalesLoading ? (
          ...
          <div className="rounded-xl bg-red-50 p-4 text-sm text-red-600">
            Failed to load today's sales report.
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {/* Total Sales */}
            <div className="rounded-xl bg-orange-50 p-4">
              ...
              <p className="mt-1 text-2xl font-bold text-gray-800">
                Birr {Number(dailySales?.totalSales ?? 0).toFixed(2)}
              </p>
            </div>
            ...
            <div className="rounded-xl bg-green-50 p-4">
              ...
              <p className="mt-1 text-2xl font-bold text-gray-800">
                Birr {Number(dailySales?.averageOrderValue ?? 0).toFixed(2)}
              </p>
            </div>
            ...
            <div className="mt-5">
              <div className="flex h-40 items-end gap-6 rounded-xl border border-gray-100 bg-white px-4 py-3 sm:gap-10">
                {[
                  {
                    label: "Dine In",
                    value: finance.dineIn,
                  },
                  {
                    label: "Takeaway",
                    value: finance.takeaway,
                  },
                ].map((bar) => (
                  <div
                    key={bar.label}
